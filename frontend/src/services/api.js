...
export const galleryAPI = {
  ...
  getArtists: async () => {
    const response = await api.get('/gallery/artists');
    return response.data;
  },

  likeArtwork: async (artworkId) => {
    const response = await api.post(`/gallery/artworks/${artworkId}/like`);
    return response.data;
  },

  deleteArtwork: async (artworkId) => {
    const response = await api.delete(`/gallery/artworks/${artworkId}`);
    return response.data;
  },
};
...
export const mlAPI = {
  ...
  enhanceImage: async (imageFile, strength = 0.6, effects = ['default']) => {
    const formData = new FormData();
    formData.append('file', imageFile);
    formData.append('strength', String(strength));
    effects.forEach(effect => formData.append('effects', effect));
    const response = await api.post('/ml/enhance', formData, {
...
export const artistsAPI = {
  getArtists: async () => {
    const response = await api.get('/artists/list');
    return response.data;
  },
  
  getArtistArtworks: async (artistId) => {
    const response = await api.get(`/artists/${artistId}/artworks`);
    return response.data;
  }
};
